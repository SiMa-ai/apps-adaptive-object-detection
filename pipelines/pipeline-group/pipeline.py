...
INSIGHT_HOST = "192.0.2.10"             # Insight, as the DevKit and browser see it
RTSP_PORT = 8554
DEVKIT = "sima@192.0.2.72"

# Insight API base: the mapped host port from the DevKit, the internal port
# from inside the SDK container.
INSIGHT_API = "https://192.0.2.10:9900" if _ON_DEVKIT else "https://127.0.0.1:9900"
...
#
#   <repository root>/
#       src/                            the detector + its labels
#       models/                         the model pack (see the README)
...
REPO = BUNDLE.parent                            # the repository root
...
#   build/adaptive-resolution-object-detector     --mode {adaptive,fused} --config X
#
...
PY_APP = REPO / "src" / "python" / "main.py"
# Produced by the CMake build; absent until someone runs it, which is why the
# UI only offers the C++ toggle once this exists.
CPP_APP = REPO / "build" / "adaptive-resolution-object-detector"
...
def cpp_binary():
    """The built C++ binary, or None when it has not been built yet."""
    return CPP_APP if CPP_APP.is_file() else None
...
def app_command() -> str:
    ...
    if language() == "cpp":
        ...
        if binary is None:
            raise SystemExit(
                "language is 'cpp' but no binary was found at "
                f"{CPP_APP}.\nBuild it from the repository root:\n"
                "  cmake -S . -B build "
                "-DCMAKE_TOOLCHAIN_FILE=cmake/aarch64-modalix.cmake\n"
                "  cmake --build build -j\"$(nproc)\"\n"
                "or switch the language toggle back to Python.")
...
# `models/` at the repository root is where the README tells you to unpack
# the model archive.
_MODEL_DIRS = (REPO / "models",)
MODELS = str(next((d for d in _MODEL_DIRS if d.is_dir()), _MODEL_DIRS[0]))
LABELS = str(REPO / "src" / "common" / "coco_label.txt")
...
model:
  path: {MODELS}/yolo_26n_mpk.tar.gz
  labels: {LABELS}
