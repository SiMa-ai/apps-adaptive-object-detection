...
#
#   ./setup-adaptive-pipeline.sh 192.0.2.10                  # host moved, same board
#   ./setup-adaptive-pipeline.sh 192.0.2.10 192.0.2.72   # fresh clone: both
#
...
usage() {
  cat <<USAGE
Usage: $(basename "$0") <host-ip> [board-ip]
       $(basename "$0") --help

Re-point this adaptive-pipeline bundle at a new set of addresses. Run it ON
the SDK container (not the DevKit) after a fresh clone, or any time this
machine's own IP changes and the pipelines stop reaching Insight or the board.

Arguments:
  <host-ip>    Required. This SDK container's IPv4 address, as the DevKit and
               your browser see it. Insight advertises this to WebRTC peers,
               so it must be reachable from both.
  [board-ip]   Optional. Your DevKit's IPv4 address. Omit it when only this
               host has moved and the board has not. Required on a FIRST run
               after cloning, because the bundle ships with whatever addresses
               the previous owner had baked in.

Both arguments must be dotted-quad IPv4 (e.g. 192.0.2.10). Hostnames are not
accepted: several of the files rewritten here cannot resolve names.

Options:
  -h, --help   Show this help and exit.

Environment overrides (all optional; each falls back to discovery):
  DEVKIT_IP     Board address, if not given positionally.
  DEVKIT_USER   SSH user on the board.                    Default: sima
  DEVKIT_ROOT   This bundle's path AS THE BOARD SEES IT. Defaults to the same
                absolute path as on the host, which is correct while the board
                NFS-mounts the container's workspace. Set it explicitly if you
                cloned outside the exported tree.
  REPO_ROOT     This bundle's path on the host. Default: this script's dir.
  NFS_EXPORT    Host directory the board NFS-mounts at /workspace. Discovered
                from the board's own fstab; set it if that lookup fails.
  INSIGHT_PORT  Insight's port as published on the host. Read from
                /etc/environment (NEAT_INSIGHT_PORT).      Default: 9900

Examples:
  # First run after cloning - both addresses must be given.
  $(basename "$0") 192.0.2.10 192.0.2.72

  # This host moved to a new network; same board as before.
  $(basename "$0") 192.0.2.55

  # Board reached as a different user, bundle cloned outside the NFS export.
  DEVKIT_USER=root DEVKIT_ROOT=/mnt/work/pipelines \\
    $(basename "$0") 192.0.2.10 192.0.2.72

This script edits BOTH machines: the container's /etc/environment, and the
board's /etc/fstab and NFS watchdog timer. It restarts Insight and the
pipeline UI servers. It is safe to run repeatedly - each run discovers the
addresses currently written into the bundle, so you never have to tell it
...
# Shape alone is not enough: a bad address is swapped across the whole bundle
# before anything tries to reach it, so an out-of-range octet would rewrite every file
# and only fail much later, during verification.
valid_ipv4() {
  local ip="$1" octet
  [[ "$ip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] || return 1
  local IFS=.
  for octet in $ip; do
    # Reject leading zeros - 010 is read as octal by some resolvers.
...
for a in "$NEW_IP" ${NEW_DEVKIT_IP:+"$NEW_DEVKIT_IP"}; do
  if ! valid_ipv4 "$a"; then
    echo "not a valid IPv4 address: $a" >&2
    exit 2
  fi
done
...
OLD_DEVKIT_IP="${OLD_DEVKIT_IP:-192.0.2.72}"
...
# ---------------------------------------------------------------------------
# These files are NOT swapped with $OLD_IP. $OLD_IP is whatever the repo
# currently says, and the repo ships documentation placeholders - so on a fresh
# clone it matches nothing here, and a find-and-replace silently does nothing
# while still exiting 0. Set the variables outright instead, whatever they hold.
set_host_var() {
  local runner="$1" file="$2" var="$3" value="$4" before after
  before="$($runner grep -oE "^[[:space:]]*(export[[:space:]]+)?${var}=.*$" "$file" 2>/dev/null || true)"
  if [[ -n "$before" ]]; then
    $runner sed -i -E \
      "s|^([[:space:]]*(export[[:space:]]+)?${var}=).*$|\\1\"${value}\"|" "$file"
  else
    printf '%s="%s"\n' "$var" "$value" | $runner tee -a "$file" >/dev/null
  fi
  after="$($runner grep -oE "^[[:space:]]*(export[[:space:]]+)?${var}=.*$" "$file" 2>/dev/null || true)"
  [[ "$before" != "$after" ]]
}

patch_env_file() {
  local runner="$1" f="$2" changed=0
  for var in CONTAINER_HOST_IP NFS_SERVER_HOST_IP; do
    set_host_var "$runner" "$f" "$var" "$NEW_IP" && changed=1
  done
  if (( changed )); then
    ok "patched $f"
  else
    ok "$f already correct"
  fi
}

# /root/.devkit-sync.rc is the one that decides what Insight advertises:
# supervisord runs neat-insight-supervised as root with HOME=/root, and that
# wrapper sources it. The /home copy is for interactive shells only. If the
# root copy is missing, create it - skipping it silently is what lets Insight
# keep serving a dead address while every other check passes.
for f in /etc/environment /root/.devkit-sync.rc; do
  if ! sudo -n test -f "$f" 2>/dev/null; then
    if ! sudo -n touch "$f" 2>/dev/null; then
      warn "$f not writable - skipped (Insight may advertise a stale address)"
      continue
    fi
  fi
  sudo -n cp "$f" "${f}.bak-previous" 2>/dev/null || true
  patch_env_file "sudo -n" "$f"
done
if [[ -f "$HOME/.devkit-sync.rc" ]]; then
  cp "$HOME/.devkit-sync.rc" "$HOME/.devkit-sync.rc.bak-previous" 2>/dev/null || true
  patch_env_file "" "$HOME/.devkit-sync.rc"
fi
...
# Anchored on the workspace mount, NOT on $OLD_IP: $OLD_IP is whatever the repo
# says, and fstab can hold a different stale address entirely - in which case a
# substitution matches nothing, reports success, and the mount breaks on the
# next reboot while every check here still passes.
if grep -q "workspace" /etc/fstab 2>/dev/null; then
  sudo -n cp /etc/fstab /etc/fstab.bak-previous
  sudo -n sed -i -E \
    "s#^[[:space:]]*([0-9]{1,3}\.){3}[0-9]{1,3}:(\S*workspace\S*)#${NEW_IP}:\2#" /etc/fstab
  echo "  fstab -> $(grep workspace /etc/fstab)"
  if ! grep -q "^${NEW_IP}:" /etc/fstab; then
    echo "  WARNING: fstab workspace line does not start with ${NEW_IP} - check it by hand"
  fi
fi
...
if (( hold == 3 )); then
  echo "  mount held: 3/3 checks over 15s"
elif mountpoint -q /workspace 2>/dev/null; then
  # The share is up but this bundle is not inside it. Cloning outside the
  # exported tree is legal - the board just cannot see this path, so the
  # pipelines cannot be launched from it. Not a flapping mount.
  echo "  mount held, but ${DEVKIT_ROOT} is not visible on the DevKit"
  echo "  (clone inside the NFS export, or set DEVKIT_ROOT= to its path on the board)"
else
  echo "  mount NOT holding: ${hold}/3 checks over 15s - the share is flapping"
fi
...
# Test reachability only. Grepping the response for $NEW_IP conflates two
# faults: a stale advertised address then reports itself as "not reachable",
# sending you after a network problem that does not exist.
if [[ "$(ssh "${SSH_OPTS[@]}" "${DEVKIT_USER}@${DEVKIT_IP}" \
     "curl -sk -o /dev/null -w '%{http_code}' --max-time 8 https://${NEW_IP}:${INSIGHT_PORT}/api/server-ip" 2>/dev/null)" == "200" ]]; then
